#!/bin/bash
# Plan B: RL 训练全流程（rollout → DPO → eval）
# 用法: cd /research/cbim/vast/mz751/Projects/DLLM-Searcher && bash recipes/run_rl_plan_b.sh
set -e
...
ROLLOUT_DIR="$ROOT/dLLM_trainer/VRPO/output/rl/plan_b/rollouts"
DPO_DATA="$ROOT/dLLM_trainer/VRPO/data/dpo_pairs_plan_b.jsonl"
TRAIN_OUT="$ROOT/dLLM_trainer/VRPO/output/rl/plan_b/model"
EVAL_OUT="$ROOT/dLLM_trainer/VRPO/output/rl/plan_b/eval_100.jsonl"
LOG_DIR="$ROOT/dLLM_trainer/VRPO/output/rl/plan_b/logs"
...
for rank in $(seq 0 7); do
    CUDA_VISIBLE_DEVICES=$rank $PYTHON \
        "$ROOT/my_eval/collect_dpo_rollouts.py" \
