...
def main():
    ...
    # Separate optimizer for cont_head (not wrapped by DeepSpeed)
    aux_optimizer = AdamW(
        cont_head.parameters(),
        lr=optimizer_config.learning_rate,
        betas=(optimizer_config.beta1, optimizer_config.beta2),
        eps=optimizer_config.epsilon,
    )
    ...
    def forward_process(noisy_input_ids, p_mask, _tok_idx_ext, labels, think_mask_b):
        ...
        # The hook below injects x_noisy = x_clean + σ*ε at think positions.
        ...
        def _embed_hook(module, args, output):
            ...
            output_noisy[think_mask_b] = (output + sig * noise)[think_mask_b]
            return output_noisy

        _handle = _embed_layer.register_forward_hook(_embed_hook)
        try:
            outputs = model(
                input_ids=noisy_input_ids,
                output_hidden_states=use_think_latent,
            )
        finally:
            _handle.remove()
        ...
        if use_think_latent and think_mask_b.any() and outputs.hidden_states is not None:
            hidden_last = outputs.hidden_states[-1]   # (B, L, d_model)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(progress_bar, start=1):
            ...
            if step % accelerator.gradient_accumulation_steps == 0:
                ...
                optimizer.step()
                aux_optimizer.step()
                # Broadcast cont_head params from rank 0 → all ranks AFTER optimizer step
                # (safe: happens after DeepSpeed's reduce-scatter is fully done)
        ...
        save_checkpoint(model, cont_head, tokenizer, config, accelerator, config.model.optimized_name, epoch=epoch)
...
def save_checkpoint(model, cont_head, tokenizer, config, accelerator, name, epoch=None):
    ...
    if accelerator.is_main_process:
        ...
        # Save cont_head separately; it's not part of the HF model state_dict
