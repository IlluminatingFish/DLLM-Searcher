...
class TrainDataset(Dataset):
    def __init__(self, extended_input_ids, p_mask, tok_idx_ext, labels, think_mask):
        ...
        self.labels = labels
        self.think_mask = think_mask  # bool (B,L): positions for x0-prediction denoising
...
def main():
    ...
    # ── x0-prediction hyperparams ─────────────────────────────────────────────
    # lambda_c weights the x0 denoising loss vs discrete CE loss.
    # sigma_min/sigma_max control the noise range injected at think positions.
    ...
    logger.info(f"x0pred config: lambda_c={lambda_c} sigma=[{sigma_min},{sigma_max}]")

    d_model = getattr(model.config, "d_model", getattr(model.config, "hidden_size", 4096))

    # x0_head: predicts clean embedding x_0 from noisy hidden state.
    # Unlike ε-prediction, x_0 is semantically meaningful — the transformer's
    # pretrained representations naturally encode semantic content, so predicting
    # the clean embedding is aligned with what the model already knows.
    x0_head = torch.nn.Linear(d_model, d_model, bias=False, dtype=torch.bfloat16)
    torch.nn.init.xavier_uniform_(x0_head.weight.float())
    x0_head = x0_head.to(accelerator.device)
    logger.info(f"x0_head: Linear({d_model},{d_model}) — {d_model*d_model/1e6:.1f}M params")

    # sigma_proj conditions the denoising on the noise level.
    # Zero-init final layer for stable warm start (starts as identity residual).
    ...
    aux_modules = [x0_head, sigma_proj]
    aux_optimizer = AdamW(
    ...
    strip_think = bool(config.dataset.get("strip_think", False))
    if strip_think:
        import re
        _think_pat = re.compile(r'<think>.*?</think>', re.DOTALL)
        response_list = [_think_pat.sub('', r) for r in response_list]
        logger.info("strip_think=True: removed <think>...</think> from all responses")
    ...
    logger.info(f"think tokens: start={think_start_ids} end={think_end_ids} use_think_latent={use_think_latent}")
    ...
    # ── Training loop ─────────────────────────────────────────────────────────
    logger.info("***** Running LLaDA x0-prediction SFT training *****")
    ...
    def forward_process(noisy_input_ids, p_mask, _tok_idx_ext, labels, think_mask_b):
        ...
        sigma = torch.empty(B, device=device).uniform_(sigma_min, sigma_max)
        eps   = torch.randn(B, L, d_model, dtype=torch.bfloat16, device=device)

        # _clean_embed stores x_0 (clean embeddings) for x0-prediction loss target.
        # Captured inside embed hook before noise injection — no extra forward pass needed.
        _clean_embed  = {}
        _final_hidden = {}

        def _embed_hook(module, args, output):
            # Save clean embeddings as the prediction target BEFORE injecting noise.
            # detach() avoids creating an extra graph through the embedding table.
            _clean_embed["x0"] = output.detach()
        ...
        logp_tok    = log_probs.gather(-1, labels.unsqueeze(-1)).squeeze(-1)
        loss_ce = -(logp_tok * p_mask_disc).sum(dim=1) / torch.tensor(
            L1_s, dtype=torch.float, device=device
        )
        loss_ce = loss_ce.sum() / B

        # x0-prediction loss: predict clean embedding x_0 from noisy hidden state.
        # The transformer's pretrained semantic representations make x_0 predictable
        # (it encodes meaning, not random noise), unlike ε-prediction which requires
        # recovering instance-specific Gaussian noise that gets washed out by 32 layers.
        loss_x0 = torch.tensor(0.0, device=device)
        if use_think_latent and think_mask_b.any() and "h" in _final_hidden and "x0" in _clean_embed:
            hidden_last = _final_hidden["h"]
            x0_pred  = x0_head(hidden_last)
            x0_clean = _clean_embed["x0"]
            loss_x0  = F.mse_loss(
                x0_pred[think_mask_b].float(),
                x0_clean[think_mask_b].float(),
            )

        return loss_ce, loss_x0
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(progress_bar, start=1):
            ...
            loss_ce, loss_x0 = forward_process(eii, pm, tie, lbl, thm)
            loss = loss_ce + lambda_c * loss_x0
            global_steps += 1
            ...
            if accelerator.is_main_process:
                wandb.log({
                    ...
                    "loss_x0_scaled":  (lambda_c * loss_x0).item(),
                }, step=global_steps)
        ...
        save_checkpoint(model, x0_head, sigma_proj, tokenizer, config, accelerator, config.model.optimized_name, epoch=epoch)
...
def save_checkpoint(model, x0_head, sigma_proj, tokenizer, config, accelerator, name, epoch=None):
    ...
    if accelerator.is_main_process:
        ...
        torch.save(x0_head.state_dict(),   save_dir / "x0_head.pt")
        ...
        logger.info(f"Saved checkpoint to {save_dir} (incl. x0_head.pt)")
