...
def main():
    ...
    if accelerator.is_main_process:
        ...
        try:
            ...
            wandb.init(mode="disabled")  # 使后续 wandb.log 不报错
    ...
    def collect_training_data(input_ids: torch.tensor, step_map_list):
        ...
        if config.training.method == "semi-ar":
            ...
            for b in range(B):
                ...
                ## Ensure that <tool_response> and </tool_response> are not masked
                n_pairs = min(len(L2_s[b]), len(L2_e[b]))
                if len(L2_s[b]) != len(L2_e[b]):
                    logger.warning(f"Batch {b}: L2_s/L2_e mismatch ({len(L2_s[b])} vs {len(L2_e[b])}), using first {n_pairs} pairs")
                for i in range(n_pairs):
                    pmask_tail[L2_s[b][i] - start_pos: L2_e[b][i] - start_pos + 1] = False
    ...
    # Filter out samples with L2_s/L2_e mismatch before collect_training_data
    L2_s_pre = find_token_positions(input_ids_lm, tokenizer.convert_tokens_to_ids('<tool_response>'), start_pos)
    L2_e_pre = find_token_positions(input_ids_lm, tokenizer.convert_tokens_to_ids('</tool_response>'), start_pos)
    valid_indices = [i for i in range(len(L2_s_pre)) if len(L2_s_pre[i]) == len(L2_e_pre[i])]
    n_filtered = len(input_ids_lm) - len(valid_indices)
    if n_filtered > 0:
        logger.info(f"Filtering {n_filtered} samples with L2_s/L2_e mismatch (keeping {len(valid_indices)} valid)")
        input_ids_lm = input_ids_lm[valid_indices]
        step_map_list = [step_map_list[i] for i in valid_indices]
    ...
    first_epoch = config.training.get("resume_epoch", 0) if config.training.get("resume_epoch") is not None else 0
    if first_epoch > 0:
        logger.info(f"Resuming training from epoch {first_epoch}")
    ...
    def forward_process(extended_input_ids, p_mask, tok_idx_ext, labels):
        ...
        L0    = start_pos
        L1    = L - L0
        ## change loss constant
        L2_s = find_token_positions(extended_input_ids[:, :L], tokenizer.convert_tokens_to_ids('<tool_response>'), start_pos)
        L2_e = find_token_positions(extended_input_ids[:, :L], tokenizer.convert_tokens_to_ids('</tool_response>'), start_pos)
        # 使用 min 避免 L2_s/L2_e 数量不匹配时越界（与 collect_training_data 逻辑一致）
        n_pairs = [min(len(L2_s[b]), len(L2_e[b])) for b in range(B)]
        L1_s = [sum([L2_e[b][i]-L2_s[b][i] + 1 for i in range(n_pairs[b])]) for b in range(B)]
        L1_s = [L1 - L1_s[b] for b in range(B)]
        # padding = [sum(extended_input_ids[b] == 151643).item() for b in range(B)]
        # L1_s = [L1_s[b] - padding[b] for b in range(B)]
        ...
        loss_lm = - (logp_tok * p_mask).sum(dim=1) / torch.tensor(L1_s, device=device)    # (B,)
