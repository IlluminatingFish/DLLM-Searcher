...
def main():
    ...
    logger.info(f"tool_response masking: {use_tool_masking}")

    # think spans ARE included in discrete CE training here —
    # the model learns to predict think-span content from context (supervised by
    # the training data's actual reasoning traces).
    ...
    use_think_ce    = bool(think_start_ids and think_end_ids)
    logger.info(f"think tokens included in CE: {use_think_ce}")
    ...
    def collect_training_data(input_ids):
        ...
        for b in range(B):
            ...
            # Exclude only tool_response spans (Agentic Noising).
            # think spans are treated as normal response tokens → included in CE.
    ...
    # ── Training loop ─────────────────────────────────────────────────────────
    logger.info("***** Running LLaDA think-CE SFT training *****")
    ...
    def forward_process(noisy_input_ids, p_mask, _tok_idx_ext, labels):
        ...
        # CE loss applied to ALL masked positions including think spans.
        log_probs = F.log_softmax(logits, dim=-1)
        logp_tok  = log_probs.gather(-1, labels.unsqueeze(-1)).squeeze(-1)
        loss_ce = -(logp_tok * p_mask).sum(dim=1) / torch.tensor(
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(progress_bar, start=1):
            ...
            loss_ce = forward_process(eii, pm, tie, lbl)
            global_steps += 1
