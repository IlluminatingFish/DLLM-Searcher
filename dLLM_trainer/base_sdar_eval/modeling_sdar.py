...
try:
    ...
    SlidingWindowCache = type("SlidingWindowCache", (), {})  # Placeholder for transformers 5.2+
...
try:
    ...
    from transformers.utils import TransformersKwargs as LossKwargs  # transformers 5.2+
...
class SDARRotaryEmbedding(nn.Module):
    def __init__(self, config: SDARConfig, device=None):
        super().__init__()
        # BC: "rope_type" was originally "type"; support rope_parameters (transformers 5.2+) and rope_scaling
        if hasattr(config, "rope_parameters") and config.rope_parameters is not None:
            self.rope_type = config.rope_parameters.get(
                "rope_type", config.rope_parameters.get("type", "default"))
        ...
        self.config = config
        # ROPE_INIT_FUNCTIONS in transformers 5.2+ does not include "default"; use custom impl
    ...
    def _compute_default_rope_parameters(config, device=None):
        """Original RoPE impl; used when rope_type='default' (not in ROPE_INIT_FUNCTIONS in transformers 5.2+)."""
        rope_params = getattr(config, "rope_parameters", None) or {}
        base = rope_params.get("rope_theta", getattr(config, "rope_theta", 10000.0))
        dim = getattr(config, "head_dim", None) or config.hidden_size // config.num_attention_heads
        # Use cpu for creation; meta device cannot hold data during init
        target_device = device if device and str(device) != "meta" else "cpu"
        inv_freq = 1.0 / (
            base ** (torch.arange(0, dim, 2, dtype=torch.int64, device=target_device).float() / dim)
        )
        return inv_freq, 1.0
...
class SDARModel(SDARPreTrainedModel):
    def __init__(self, config: SDARConfig):
        ...
        self.padding_idx = getattr(config, 'pad_token_id', None) or getattr(config, 'eos_token_id', 0)
