...
OUT_PATH        = "/research/cbim/vast/mz751/Projects/DLLM-Searcher/dLLM_trainer/Agentic/output/base_analysis_notool_results.jsonl"
...
def main():
    ...
    for i, case in enumerate(cases):
        ...
        # ── 过滤掉内容以 <tool_response> 开头的 user message（即搜索结果）───────
        def strip_tool_response(msgs):
            filtered = []
            for m in msgs:
                content = m.get("content", "")
                if m["role"] == "user" and content.strip().startswith("<tool_response>"):
                    continue   # 跳过搜索结果
                filtered.append(m)
            return filtered

        clean_msgs = strip_tool_response(orig_msgs)
        n_removed  = len(orig_msgs) - len(clean_msgs)

        # 构建输入：去掉 tool_response 的对话 + 分析请求
        msgs_with_analysis = clean_msgs + [{"role": "user", "content": ANALYSIS_MSG}]
        ...
        print(f"  去掉 tool_response 条数: {n_removed}")
