#!/bin/bash
# Plan B: DPO + x0pred regularization on hermes (8 GPU)
...
LOG_FILE=output/rl/plan_b/train_$(date +%Y%m%d_%H%M%S).log

mkdir -p output/rl/plan_b
mkdir -p "$OUTPUT_DIR"

echo "[plan_b] 启动训练: $(date)" | tee "$LOG_FILE"
...
echo "[plan_b] 训练完成: $(date)" | tee -a "$LOG_FILE"
