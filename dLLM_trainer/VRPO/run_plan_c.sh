#!/bin/bash
# Plan C: Latent ELBO DPO on hercules (8 GPU)
...
LOG_FILE=output/rl/plan_c/train_$(date +%Y%m%d_%H%M%S).log

mkdir -p output/rl/plan_c
mkdir -p "$OUTPUT_DIR"

echo "[plan_c] 启动训练: $(date)" | tee "$LOG_FILE"
...
echo "[plan_c] 训练完成: $(date)" | tee -a "$LOG_FILE"
