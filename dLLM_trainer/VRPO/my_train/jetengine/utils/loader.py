...
def load_from_hf_peft_model(target_model: nn.Module, hf_model: nn.Module):
    """
    Load weights from a HuggingFace transformer model to the custom model.
    ...
    loaded_params = set()
    missing_params = []
    hf_model_copy = copy.deepcopy(hf_model)
    # hf_merged_model = hf_model_copy.merge_and_unload()
    # Build a new state dict with materialized tensors
    hf_merged_model = hf_model
    new_state_dict = {}
    ...
    for name, param in target_model.named_parameters():
        with torch.no_grad():
            ...
            if 'qkv_proj.weight' in name:
                ...
                try:
                    q_weight = hf_merged_model.get_parameter(q_name)
                    k_weight = hf_merged_model.get_parameter(k_name)
                    v_weight = hf_merged_model.get_parameter(v_name)
            ...
            # Handle fused gate_up_proj (non-MoE)
                ...
                try:
                    gate_weight = hf_merged_model.get_parameter(gate_name)
                    up_weight = hf_merged_model.get_parameter(up_name)
            ...
            # Handle regular parameters (direct mapping)
                ...
                try:
                    hf_param = hf_merged_model.get_parameter(name)
                    ...
                    if name.startswith('model.'):
                        try:
                            hf_param = hf_merged_model.get_parameter(name[6:])
