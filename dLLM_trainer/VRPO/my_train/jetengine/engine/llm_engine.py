...
class LLMEngine:
    def __init__(self, model, **kwargs):
        # Optional external tokenizer support
        external_tokenizer = kwargs.pop("tokenizer", None)
        ...
        self.model_runner = ModelRunner(config, local_rank, self.events)
        if external_tokenizer is not None:
            self.tokenizer = external_tokenizer
        else:
            # Determine a source to construct the tokenizer
            tok_source = None
            if isinstance(config.model, str):
                tok_source = config.model
            else:
                tok_source = config.tokenizer_name_or_path or getattr(config.hf_config, "name_or_path", None)
            assert tok_source is not None, "Tokenizer source could not be determined; pass tokenizer=... explicitly"
