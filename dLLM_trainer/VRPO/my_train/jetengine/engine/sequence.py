...
class Sequence:
    ...
    def commit_block(self, block_tokens: list[int]):
        ...
        for i, token_id in enumerate(block_tokens):
            global_position = self.num_tokens + i
            is_generated = global_position >= self.num_prompt_tokens
            if is_generated and not self.ignore_eos and (token_id == self.eos_token_id or token_id in self.stop_words):
                final_block.append(token_id)
                k += 1
                self.status = SequenceStatus.FINISHED
                break
            if is_generated and self.num_completion_tokens + k >= self.max_tokens:
                self.status = SequenceStatus.FINISHED
                break
            final_block.append(token_id)
            k += 1
