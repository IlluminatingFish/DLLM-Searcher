...
from jetengine.utils.loader import load_model, load_from_hf_model,load_from_hf_peft_model
...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        # Resolve dtype: SDAR uses "dtype", HF uses "torch_dtype"
        torch_dtype = getattr(hf_config, 'torch_dtype', None) or getattr(hf_config, 'dtype', None)
        ...
        # FlashAttention only supports fp16/bf16; force bf16 when config has fp32
        ...
        # Load weights either from path or directly from a provided HF model
        if isinstance(config.model, str):
            load_model(self.model, config.model)
        elif config.hf_model is not None:
            load_from_hf_model(self.model, config.hf_model)
            # load_from_hf_peft_model(self.model, config.hf_model)
        else:
            raise ValueError("Model weights source not provided: pass a model path (str) or an HF model instance")
        # Convert to bf16 if needed (FlashAttention requires fp16/bf16)
        if next(self.model.parameters()).dtype != hf_config.torch_dtype:
            self.model = self.model.to(hf_config.torch_dtype)
    ...
    def allocate_kv_cache(self):
        ...
        print(f"total: {total}, used: {used} : {config.gpu_memory_utilization} %")
