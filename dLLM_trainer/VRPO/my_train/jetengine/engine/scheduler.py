                ...
                if seq.status == SequenceStatus.DENOISING:
                    ...
                    if seq.remasking_strategy == 'sequential':
                        ...
                        if seq.current_denoising_step == 0:
                            ...
                            target_position = 126
                            target_token_id = 151658
                            seq_x0[target_position] = target_token_id
                            transfer_index[target_position] = True
                            target_position = 63
                            target_token_id = 151657
                            seq_x0[target_position] = target_token_id
                            transfer_index[target_position] = True
                        else:
                            confidence = torch.where(mask_index, seq_x0_p, -np.inf)
                    ...
                    elif 'answer_rl' in seq.remasking_strategy:
                        if seq.current_denoising_step == 0:
                            seq_x0[63] = 151648
                            transfer_index[63] = True
                            seq_x0[126] = 151649
                            transfer_index[126] = True
                        else:
                            confidence = torch.where(mask_index, seq_x0_p, -np.inf)
                            confidence[63:] = confidence[63:] + 0.5
                            _, top_indices = torch.topk(confidence, num_to_transfer)
