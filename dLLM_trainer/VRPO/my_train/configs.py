...
class GRPOConfig(trl.GRPOConfig):
    ...
    denoising_steps: int = field(
        default=4,
        metadata={"help": "Number of denoising steps for diffusion models."},
    )
    remasking_strategy: str = field(
        default="low_confidence_dynamic",
  )
    confidence_threshold: float = field(
        default=0.85,
        metadata={"help": "Confidence threshold for remasking."},
    )
    use_jet_engine: bool = field(
        default=False,
        metadata={"help": "Whether use jet_engine for SDAR sampling"},
    )
    difficulty_threshold: Optional[float] = field(
        default=None,
        metadata={"help": "Difficulty threshold for filtering datasets."},
    )
