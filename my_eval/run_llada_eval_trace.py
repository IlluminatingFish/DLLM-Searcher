#!/usr/bin/env python3
"""
run_llada_eval_trace.py
=======================
完全复用 run_llada_eval.py 的推理逻辑（ChatML format, agentic rollout），
在 denoise_block 里额外记录每个 token 在第几步被揭示（reveal_step / reveal_conf），
输出供 report 可视化使用的 JSONL。

用法:
  TRITON_CACHE_DIR=/tmp/triton_cache python my_eval/run_llada_eval_trace.py \
    --gpu 3 --n 20 \
    --out /tmp/.../token_order_eval_20.jsonl
"""

import argparse, json, os, re, sys, torch, requests
...
DEFAULT_OUT = "/tmp/claude-1905237802/-research-cbim-vast-mz751-Projects-DLLM-Searcher-dLLM-trainer-VRPO/bc4244b6-9acd-4dae-a2de-99cb6ecf18de/scratchpad/token_order_live_20.jsonl"

# ── Search API（与 run_llada_eval.py 完全一致）────────────────────────────────
def _load_config():
    for path in [
...
# ── Prompts（与 run_llada_eval.py 完全一致）───────────────────────────────────
...
# ── denoise_block_trace：与 run_llada_eval.py 相同逻辑，加 reveal 记录 ─────────
@torch.no_grad()
...
def llada_generate_trace(model, tokenizer, prompt_ids, mask_id,
                         stop_strings=None, max_blocks=16):
    """与 llada_generate_blocks 相同逻辑，但返回 (text, blocks_trace)。"""
...
# ── run_one_trace：与 run_one 完全相同，加 blocks_trace 记录 ──────────────────
def run_one_trace(model, tokenizer, mask_id, question):
    """真实调用 Serper search API，与 run_llada_eval.py 完全一致。"""
    ...
    for turn in range(MAX_TURNS):
        ...
        print(f"    turn {turn+1} tc={is_toolcall} ans={has_answer} force={force_answer} "
        ...
        if queries:
            # 真实调用 Serper search API，并记录结果
            search_result = run_search(queries)
            turn_entry["search_result"] = search_result   # ← 记录到 trace
            print(f"    → live search #{tool_resp_idx+1}: {queries}  ({len(search_result)} chars)")
            tool_resp_idx += 1
...
def main():
    ...
    parser.add_argument("--offset", type=int, default=0,  help="从第几道题开始（0-indexed）")
    parser.add_argument("--n",   type=int, default=20)
    parser.add_argument("--out", type=str, default=DEFAULT_OUT)
    parser.add_argument("--input", type=str, default=DATA20_PATH, help="输入数据文件（.json 或 .jsonl）")
    ...
    if args.input.endswith(".jsonl"):
        all_data = [json.loads(l) for l in open(args.input)]
    else:
        all_data = json.load(open(args.input))
    data20 = all_data[args.offset : args.offset + args.n]

    print(f"[load model] {PI3_PATH}")
    tokenizer = AutoTokenizer.from_pretrained(PI3_PATH, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(
        PI3_PATH, trust_remote_code=True, torch_dtype=torch.bfloat16
    ).cuda().eval()
    model.config.use_cache = False
    mask_id = model.config.mask_token_id
    print(f"[ready] mask_id={mask_id}")
    ...
    for qi, qdata in enumerate(data20):
        ...
        result = run_one_trace(model, tokenizer, mask_id, q)

        pred = result["prediction"] or ""
        regen_correct = gt.lower().strip() in pred.lower() if pred else False
        ...
        entry = {
            ...
            "orig_correct": qdata["correct"],
            "termination_reason": result["termination_reason"],
            "turns":   result["turns"],
        }
        out_f.write(json.dumps(entry, ensure_ascii=False) + "\n")
        out_f.flush()
        print(f"  → pred={repr(pred[:60])}  correct={regen_correct}  reason={result['termination_reason']}")
